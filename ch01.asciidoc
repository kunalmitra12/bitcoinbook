...
** Bitcoin is a collection of concepts and technologies that form the basis of a digital money ecosystem, while bitcoin refers to one unit of the currency
** The Bitcoin protocol stack refers to the open source instructions of how any computing device connected to the internet can communicate with other nodes on the Bitcoin network, in the same way it uses the TCP/IP protocol to communicate with other nodes on the Internet
** Bitcoin can be purchased, sold, and exchanged for other currencies at specialized currency exchanges in a fast, secure, and borderless manner
** Users of Bitcoin own keys that prove ownership of bitcoin in the Bitcoin network
*** Keys are often stored in a digital wallet and are used to sign transactions in order to spend bitcoin, so the control is entirely in the hands of the holder of the keys (remember, *not your keys, not your bitcoin*)
** Bitcoin is a distributed, peer-to-peer system with no centralized entity in control of the network, so bitcoins are created by "mining"
*** Mining involves using computing power to find solutions to a mathematical problem which validates transactions that are broadcasted on the network
**** Every 10 minutes (on average-no matter the current hash rate), a Bitcoin miner validates the tranasactions in a block and is rewarded with Bitcoin by adding a transaction on the block that says "This miner owns *current block reward* bitcoin"
***** Remember, at the end of the day, Bitcoin is simply an distributed ledger that is incorruptible because it uses mining as a "Proof of Work" consensus algorithm
** The Bitcoin protocol halves the rate at which new bitcoin is created every 4 years to prevent inflation (the FED could never lol), *limiting the total supply of Bitcoin to just below 21 million bitcoins*, with the last bitcoin to be mined somewhere in 2140
** Bitcoin consists of: 
...
** Bitcoin is a culmination of cryptography to provide a more sound version of money that fixes the counterfeit problem of fiat cash, the "double-spend" problem of digital money, and prevents anyone else from claiming ownership over someone else's funds (Private keys)
*** No way to counterfeit bitcoin without controlling over 51% of the network
*** Once the transaction is validated on the blockchain, the transaction is final, on the blockchain, *forever*

* History of Bitcoin
** Bitcoin was invented in 2008 with the publication of the paper: "Bitcoin: A Peer-to-Peer Electronic Cash System" written under the alias of Satoshi Nakamoto
** Bitcoin network started in 2009 based on a reference implementation by Nakamoto and since revised by many other programmmers
** Largest transaction processed so far was $1.1 billion transmitted instantly and processed for a fee of only $0.68.
** Satoshi withdrew from the public in 2011 leaving the responsibility of developing the code to a thriving group of volunteers, and his/her/their identity is unknown
** Nakamoto's invention is a practical and novel solution to the Byzantine Generals' Problem with the Proof-of-Work algorithm

* Bitcoin Uses, Users, and Their Stories
** Bitcoin is a protocol that can be accessed using a client application that speaks the protocol
*** "Bitcoin wallet" is a wallet is the most common user interface to the Bitcoin system, just like a web browser is the most common user interface for the HTTP protocol
**** Desktop wallet - first wallet created as reference implementation, but running on Windows or Mac OS has certain security disadvantages
**** Mobile wallet - most common, easy to use
**** Web wallet - accessed through web browswer and store user's wallet on server owned by third party, some services operate using client-side code running in user's browser, keeping control of keys in the hands of the user, but most compromise by taking control of keys in exchange for ease-of-use
**** Hardware wallet - devices that operate a secure self-contained Bitcoin wallet on special purpose hardware, all Bitcoin-related operations are handled on the specialized hardware (*very secure*)
*** There is also a reference implementation of  the Bitcoin protocol that includes a wallet, known as the "Satoshi Client" or "Bitcoin Core," derived from the original implementation written by Satoshi Nakamoto
**** Full-node client - stores entire blockchain, manages users' wallets, and can initiate transactions directly on the Bitcoin network (need ~350 GB for blockchain and 2 GB of RAM, but offers complete autonomy and independent transaction verification)
**** Lightweight client - also knowns as simplified-payment-verification (SPV) client, connects to Bitcoin full nodes to access transaction information, stores user wallet locally, and creates/validates/transmits transactions, can interact directly with Bitcoin network without intermediary
**** Third-party API client - interacts with Bitcoin through a third-party system of APIs, rather than connecting to the Bitcoin network directly
** Mnemonic Words - most modern Bitcoin wallets will provide a phrase of 12-24 English words to act as a recovery phrase in case you lose access to your wallet (write down 2 copies, keep safe in locker)

* Alice's Journey
** When Alice uses her new bitcoin wallet and buys new bitcoin, her wallet randomly generates a private key which is used to derive Bitcoin addresses that direct to her wallet
...
** Anytime you buy bitcoin, KYC is involved
** Each bitcoin is divided into 100 million subunits, each called a "satoshi" - *divisble*
